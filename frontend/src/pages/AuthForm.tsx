    ...
    const onSubmit = async(data: {email: string, password: string}) => {
        if (mode === 'login') {
            ...
            try {
                ...
                setServerError("Регистрация не удалась, попробуйте другой email");
            }
        }
    }
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 flex flex-col gap-6">
                    ...
                    <div className="flex flex-col">
                        <Input type="email" placeholder="Email" {...register("email")}
                            className="w-96 h-12 border-[#634b20] border-1 rounded-md pl-3 focus:outline-none"
                        />
                        {errors.email && <small className="text-sm text-red-500 mt-1">{errors.email.message}</small>}
                    </div>
                    ...
                    <div className="flex flex-col">
                        <Input type="password" placeholder="Пароль" {...register("password") } 
                            className="w-96 h-12 border-[#634b20] border-1 rounded-md pl-3 focus:outline-none"
                        />
                        {errors.password && <small className="text-sm text-red-500 mt-1">{errors.password.message}</small>}
                    </div>
                    ...
                    {mode === 'register' ? (
                        <div className="flex flex-col">
                            <Input type="password" placeholder="Повторите пароль" {...register("confirmPassword")} 
                                className="w-96 h-12 border-[#634b20] border-1 rounded-md pl-3 focus:outline-none"
                            />
                            {errors.confirmPassword && <small className="text-sm text-red-500 mt-1">{errors.confirmPassword.message}</small>}
                        </div>
                    ) : (
                    ...
                    <div className="flex flex-col">
                        {serverError && <small className="text-sm text-red-500 mt-1">{serverError}</small>}
                        <Button type="submit" className="cursor-pointer w-full h-12 text-white hover:bg-gray-900 bg-gray-950 rounded-md mt-2" >
