...
export default function Profile() {
    const user = useParams<{nickname: string}>();
    const decodedNickname = user.nickname ? decodeURIComponent(user.nickname) : '';

    const isMe = useRef<{data: boolean}>({data: false});
    ...
    const [errors, setErrors] = useState({} as {nickname?: string})
    ...
    useEffect(() => {
        API.get(`/api/user/${decodedNickname}`)
        .then(res => {
            ...
            isMe.current.data = localStorage.getItem('nickname') === res.data.nickname;
    ...
    const handleUpdate = async(e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        try {
            const res = await API.put('/api/me', {avatar, nickname});
            if (res.status !== 201) {
                setErrors({nickname: res.data.error});
            } else {
                setErrors({});
                setEditMode(false);
            }
                ...
                <div className="w-full flex min-h-screen h-full flex-col gap-8 items-center px-4">
                    {editMode && localStorage.getItem('nickname') === nickname ? (
                        <form className="bg-white w-full max-w-4xl rounded-2xl p-10 shadow-xl flex flex-col gap-6" onSubmit={e => handleUpdate(e)}>
                            <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
                                <div className="relative group cursor-pointer max-w-48 w-full">
                                    <img
                                ...
                                <div className="flex flex-col gap-6 w-full">
                                    <div className="space-y-2">
                                        ...
                                        <div className="relative">
                                            <Input 
                                                className={
                                                    'w-full h-12 border-gray-700 border-1 rounded-md pl-3 text-gray-700 focus:outline-none'
                                                }
                                                value={nickname}
                    ...
                    <div className="bg-white w-full max-w-4xl rounded-2xl p-10 shadow-xl flex flex-col gap-6 justify-center items-center">
                        ...
                        <div className="w-full flex flex-col gap-8 items-center">
                            {posts.map((post: Post) => (
                               <PostContainer post={post} mine={isMe.current.data} removePostFromState={removePostFromState} key={post.id}/>
                            ))}
                        </div>
                    </div>
                </div>
